...
<br /><font color="#3333FF" size="3">KASIR APOTEK TBS</font><br /><br />
<form name="jumlah" action="?page=penjualan_insert" method="post" onsubmit="return validate_form();">
<table border="0">
	<tr>
		<td>Jumlah</td>
		<td>:</td>
		<td><input type="text" name="qty" value="<?php echo 1;// $_POST['qty'];?>" autocomplete="off" placeholder="harus diisi..!"></td>
	</tr>
	<tr>
		<td>Nama obat</td>
		<td>:</td>
		<td><input type='text' onKeyUp="autoComplete();" id='inputan' name="kode_obat" autofocus autocomplete="off"/>
...
if(isset($_POST['insert_penjualan'])){ //script ini dilakukan saat button bernama"confirm" ditekan
	...
	if($bayar < $totalharga){ // JIKA UANG PEMBAYARAN TIDAK MENCUKUPI
		...
		<br /><br />
		No Kartu : <input type="text" name="no_kartu" autocomplete="off" value="0"/>
		Cash = <input type="text" name="bayar" placeholder="harus diisi..!" />
		<input type="submit" value="CONFIRM" name="insert_penjualan" />			
		</form>
...
?>
<br /><br />
<form name="bayar" action="?page=penjualan_insert" method="post">
